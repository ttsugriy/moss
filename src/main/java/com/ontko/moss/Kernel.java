...
public class Kernel extends Thread {
    ...
    // Field responsible for replacement algorithm
    private ReplacementAlgorithm.Names replacementAlgorithm = ReplacementAlgorithm.Names.DEFAULT;

    public ReplacementAlgorithm.Names getReplacementAlgorithm () {
        return this.replacementAlgorithm;
    }

    public void setReplacementAlgorithm( ReplacementAlgorithm.Names algorithm ) {
	...
	public void init(String commands, String config) {
					...
					if (tmp.startsWith("random")) {
						...
						if (0 > addr || addr > address_limit) {
							System.out.println("MemoryManagement: " + addr
									+ ", Address out of range in " + commands);
							System.exit(-1);
						}
						instructVector
		...
		if (map_count < (virtPageNum + 1) / 2 && false) {
			for (i = 0; i < virtPageNum; i++) {
				Page page = memVector.elementAt(i);
		...
		for (i = 0; i < instructVector.size(); i++) {
			...
			if (instruct.addr < 0 || instruct.addr > high) {
				System.out.println("MemoryManagement: Instruction ("
						+ instruct.inst + " " + instruct.addr
						+ ") out of bounds.");
				System.exit(-1);
			}
		}
		...
		if (instruct.inst.startsWith("READ")) {
			...
			if (page.physical == -1) {
				...
				PageFault.replacePage(memVector, virtPageNum, Virtual2Physical
						...
						controlPanel, getReplacementAlgorithm());
